...
int audio_open_ctrl_path()
{
    ...
    {
        ALOGW("control path opened successfull");
        return 0;
    }
    else
        //INFO("control path open failed");
        ALOGW("control path open failed");
    return -1;
}
...
int wait_for_stack_response(uint8_t time_to_wait)
{
    ...
    while (retry < CTRL_CHAN_RETRY_COUNT &&
              ack_recvd == 0)
    {
        ALOGW("enterin coditional wait: retry = %d, ack_recvd = %d",retry,ack_recvd);
    ...
    if (ack_recvd) {
        ALOGW("wait_for_stack_response: ack received");
    }
    ALOGW("wait_for_stack_response returning retry = %d",retry);
    return retry;
}
                                  ...
                                  size_t len)
{
    ...
    if (status != A2DP_CTRL_ACK_PENDING)
    {
        ...
        if (status == A2DP_CTRL_ACK_SUCCESS)
        {
            ...
            for (int i = 0; i < len; i++) {
                ALOGW("audio_stream.codec_cfg[%d] = %x",i,audio_stream.codec_cfg[i]);
            }
            if (!ack_recvd)
            {
                ack_recvd = 1;
    ...
    //TO copy codec config and unlock conditional wait
}
    ...
    for (j = 0; j <STREAM_START_MAX_RETRY_LOOPER; j++) {
        for (i = 0; i < STREAM_START_MAX_RETRY_COUNT; i++)
        {
            ...
            if (stack_cb)
            {
                ...
                {
                    ALOGW("a2dp stream start failed: call in progress");
                    goto end;
                }
        ...
        //if (stack_cb->get_connection_status_cb() != A2DP_CTRL_ACK_SUCCESS) {
        ...
        if (status != A2DP_CTRL_ACK_SUCCESS)
        {
            //TODO lock it and waith for stack return with status
